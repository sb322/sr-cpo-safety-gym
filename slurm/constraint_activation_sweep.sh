#!/bin/bash -l
#SBATCH --job-name=sr_cpo_constr
#SBATCH --output=safe_constraint.%A_%a.out
#SBATCH --error=safe_constraint.%A_%a.err
...
CONSTRAINT_LABELS=("unconstrained" "cl1e-4" "cl5e-5" "cl1e-5" "cl0")
COST_LIMITS=("0.0001" "0.0001" "0.00005" "0.00001" "0.0")
PID_KPS=("0.0" "5.0" "5.0" "5.0" "5.0")
PID_KIS=("0.0" "0.1" "0.1" "0.1" "0.1")
PID_KD="0.0"
TASK_ID="${SLURM_ARRAY_TASK_ID:-0}"
CONSTRAINT_LABEL="${CONSTRAINT_LABELS[$TASK_ID]}"
COST_LIMIT="${COST_LIMITS[$TASK_ID]}"
PID_KP="${PID_KPS[$TASK_ID]}"
PID_KI="${PID_KIS[$TASK_ID]}"
SEED="0"
...
echo "CONSTRAINT_LABEL=$CONSTRAINT_LABEL"
...
# 2. Probes wired through (forward NaN, grad/param, actor-component, PID)
for marker in [
    ...
    "pid_error", "pid_integral", "pid_raw_lambda",
]:
    assert marker in src_losses or marker in src_train, f"probe missing — {marker}"

# 3. Phase-1g alpha cap and calibrated default budget
