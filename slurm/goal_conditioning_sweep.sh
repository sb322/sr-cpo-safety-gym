#!/bin/bash -l
#SBATCH --job-name=sr_cpo_goal
#SBATCH --output=safe_goal.%A_%a.out
#SBATCH --error=safe_goal.%A_%a.err
...
GOAL_LABELS=("full_d8" "lidar_d8" "full_d16" "lidar_d16")
NUM_BLOCKS=("8" "8" "16" "16")
GOAL_STARTS=("0" "16" "0" "16")
GOAL_DIMS=("55" "16" "55" "16")
TASK_ID="${SLURM_ARRAY_TASK_ID:-0}"
GOAL_LABEL="${GOAL_LABELS[$TASK_ID]}"
NUM_BLOCK="${NUM_BLOCKS[$TASK_ID]}"
...
# 2. Probes wired through (forward NaN, grad/param, actor-component, PID, goal progress)
for marker in [
...
# 3. Phase-1g alpha cap, calibrated default budget, residual switch, and goal slicing
...
# of obs/cost/progress. Catches env-construction bugs before training.
