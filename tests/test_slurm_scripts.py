...
def test_slurm_smoke_has_wulver_header_and_static_gate() -> None:
    source = Path("slurm/smoke.sh").read_text()
...
def test_slurm_smoke_static_diff_heredoc_passes_locally() -> None:
    block = _static_check_block("slurm/smoke.sh")
